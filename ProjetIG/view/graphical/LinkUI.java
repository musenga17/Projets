...
public abstract class LinkUI {
    ...
    public Text poids;
    EventHandler<MouseEvent> showMenu, confirm, moved, exited; //confirm : confirmer la modification du poids par un clic sur la forme, moved : pour la surbrillance lorsqu'on place la souris dessus , exited : remettre en noir quand on enleve la souris
	...
	edit_enabled.addListener((observer,oldValue,newValue) -> {
		if(newValue) {
		    ...
		    for (SommetUI sui : IG.listSommet) { // On ne veut plus pouvoir agir sur les sommets lors de la modification d'un poids
	...
	confirm = new EventHandler<MouseEvent>() {
		...
		public void handle(MouseEvent e) {
			...
			if (s.length() > 0) { // Si = 0 pas de confirmation
	...
	supprHandler = new EventHandler<KeyEvent>() {
		...
		public void handle(KeyEvent e) {
		    String s = poids.getText();
		    setPoidsProperties();
		    if (e.getCode() == KeyCode.BACK_SPACE && s.length() > 0) { //Pour pas pouvoir effacer quand mot vide
...
public void setMouseActions(){ //pour créer et ajouter le moved et le exited 
