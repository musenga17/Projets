...
public class Arete extends LinkUI {
	...
	if (S1 instanceof Circle) {
	    if (S2 instanceof Circle) {
		poids.xProperty().bind(Bindings.add(((Circle)S1).centerXProperty(),((Circle)S2).centerXProperty()).divide(2));//x du texte de l'arete <-- (x du centre de s1 + x du centre de S2)/2
		poids.yProperty().bind(Bindings.add(((Circle)S1).centerYProperty(),((Circle)S2).centerYProperty()).divide(2));//idem avec y
	    }
	    if (S2 instanceof Rectangle) {
