...
public class Graphe extends Model {
    ...
    public ArrayList<ArrayList<Link>> stepsDijkstra = new ArrayList<ArrayList<Link>>();// La liste des chemins pour dijkstra du sommet de depart vers tous les autre
    public ArrayList<Link> stepsKruskal = new ArrayList<Link>();// La liste des aretes pour l'arbre couvrant minimum
    public ArrayList<Link> stepsPrim = new ArrayList<Link>();// idem
	...
	while (sommetsTmp.size() > 0) {
		...
		int vxv = getCout(x,v);
		if (deltaY > deltaX + vxv) { //S'il existe un chemin plus court que le précédent
    ...
    public void displayStepKruskal(int indice, boolean b) { //parcours en arriere -> false pour noir / en avant -> true pour rouge
	...
	Collections.sort(aretes, new Comparator<Link>(){
		...
		public int compare(Link l1, Link l2) {
		    return ((Integer)l1.poids).compareTo(l2.poids); //Pour trier les aretes par poids
		}
	    });
	for (Link l : aretes) {
	    Sommet st = l.start;
	...
	updateObserver(stepsKruskal.size()); //Envoie a l'interface graphique le nombre d'etape pour savoir quand desactiver le bouton next
    }
	...
	HashMap<Sommet,Sommet> pred = new HashMap<Sommet,Sommet>(); // pred [0:nil 1:nil 2:nil 3:nil]
	for (Sommet so : sommets) {
	    pred.put(so,null);
	}
	s.cout = 0;
		...
		if (file.contains(v) && v.cout > getCout(t,v)) {
			...
			Collections.sort(file, new Comparator<Sommet>() {
				...
				public int compare(Sommet s1, Sommet s2) {
				    return ((Integer)s1.cout).compareTo(s2.cout); //tri par rapport au cout du sommet
				}
			    });
		    }
		}
